...
- **Linha Investimento ligada à carteira**: o `realValue` da linha INVESTMENT reflete o quanto foi investido no período (soma dos `PositionEntry` da carteira com data ≥ início do período = `resetDay` mais recente). Finance depende do port `InvestedLookup` (domain), implementado por `WalletInvestedLookup` (infra, lê `WalletRepository`); `getCurrentPeriod` sincroniza a linha e expõe `investedThisMonth`. No front a célula "real" da linha vira somente-leitura ("da carteira")
