...
- **Frente atual:** módulo `finance` — feito: passe de correção/segurança (IDOR de renda, auto-close, mês de arquivamento, retenção 12m, link recorrente, status), DTOs de resposta + validação, testes, **limites de gasto por categoria com alerta**, **modelo de recorrentes mais rico** (dueDay/variável + rendas recorrentes) e **comparação mensal (MoM)** no histórico. Próximo: integração da linha Investimento com a carteira (semântica já definida — ver memória: buys do mês). Ainda em aberto: categorias custom (migração), CSV, i18n
