...
public class WalletService {
    ...
    public Optional<WalletSummaryDTO> findById(String id) {
        return walletRepository.findById(id)
                .map(wallet -> walletMapper.toSummaryDTO(wallet, resolvePrices(wallet)));
    }
    ...
    private Map<String, BigDecimal> resolvePrices(Wallet wallet) {
        return wallet.positions().stream()
                .collect(Collectors.toMap(
                        p -> p.ticker(),
                        p -> stockPriceLookup.findCurrentPrice(p.ticker()).orElse(BigDecimal.ZERO),
                        (a, b) -> a
                ));
    }
}
